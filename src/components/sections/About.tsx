...
export function About() {
  ...
  // Mobile swipe: left swipe triggers the split animation
  useEffect(() => {
    ...
    const onTouchEnd = (e: TouchEvent) => {
      ...
      if (!split && dx < 0) {
        // Swipe left — split open
        split = true;
        left.style.transition  = "transform 0.5s cubic-bezier(0.16,1,0.3,1)";
        right.style.transition = "transform 0.5s cubic-bezier(0.16,1,0.3,1)";
        left.style.transform   = "translateX(-100%)";
        right.style.transform  = "translateX(100%)";
      } else if (split && dx > 0) {
        // Swipe right — close back
        split = false;
  ...
  const mobileStyles = `
    @media (max-width: 768px) {
      ...
      .about-center-line { display: none !important; }
      .about-swipe-hint {
        display: flex !important;
      }
    }
    @media (min-width: 769px) {
      .about-swipe-hint { display: none !important; }
    }
  `;
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Mobile swipe hint */}
          <div className="about-swipe-hint" style={{ display: "none", alignItems: "center", gap: 6, marginTop: 8 }}>
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path d="M10 8H2M6 4l-4 4 4 4" stroke="#47b76e" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M13 5v6" stroke="#47b76e" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/>
            </svg>
            <span style={{ fontSize: 11, color: "#47b76e", letterSpacing: "0.06em", fontFamily: "Inter, sans-serif" }}>Swipe left to reveal</span>
          </div>
        </div>

        {/* CENTER hairline — hidden on mobile */}
        <div className="about-center-line" style={{
